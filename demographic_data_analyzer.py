...
def calculate_demographic_data(print_data=True):
    ...
    country_high_income_percentage = df.groupby('native-country').apply(
    lambda x: (x[x['salary'] == '>50K'].shape[0] / x.shape[0]) * 100)
    highest_earning_country_percentage = round(country_high_income_percentage.max(), 1)
    highest_earning_country = country_high_income_percentage.idxmax()
